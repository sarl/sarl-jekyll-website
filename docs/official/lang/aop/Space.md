...
Before reading this document, we recommend that you read the [Expression Syntax Reference](../expr/index.html),
...
[built-in capacity reference](./Behaviors.html) for details).
...
Below, the implementation extends one of the abstract classes provided by the [Janus Platform](http://www.sarl.io/runtime/janus/).
...
class PhysicSpaceImpl extends AbstractSpace implements PhysicSpace {
	...
	def forEachStrongParticipant(lambda : (UUID) => void) {
		synchronized (this.entities) {
	...
	def forEachWeakParticipant(lambda : (UUID) => void) {
	}
}
```
...
The functions `getNumberOfStrongParticipants`, `getNumberOfWeakParticipants`, `isPseudoEmpty`, `forEachStrongParticipant` and `forEachWeakParticipant` are declared in the inherited supertype `Space`.
They corresponds to:

* `getNumberOfStrongParticipants`: The reply of the number of participants to the physical space;
* `getNumberOfWeakParticipants`: always zero because there is no weak participant in the physical space;
* `isPseudoEmpty`: indicates if the physical space has no participant or the single participant has the ientifier given as argument;
* `forEachStrongParticipant`: Call the lambda provided as argument for each of the weak participants in the physical space;
* `forEachStrongParticipant`: Do nothing because there is no weak participant in the physical space.
