...
The SARL Run-time Environment (SRE) is part of the [SARL toolchain](./toolchain/index.html).
...
Parallel execution of the pro-active behaviors of an agent is supported by the tasks that are launched with the [Schedules](./aop/Schedules.html)
built-in capacity.
...
A [Behavior](./aop/Behavior.html) is the specification of a collection of behavior units (or event handlers).
...
Since a behavior must be registered as explained in the [Behaviors built-in capacity](./aop/Behaviors.html), the agent is managing internally
a collection of behaviors.

In the sequence diagram below, you could see that a behavior is created and then registered through the [Behaviors built-in capacity](./aop/Behaviors.html).
...
This keyword is formally defined in a dedicated [reference page](./expr/Synchronization.html).
...
Regarding the sending of the events, SARL provides [built-in capacities](./aop/BIC.html) that assume the sending is done asynchronously.
...
The spawning is supported by the [Lifecycle built-in capacity](./aop/Lifecycle.html) that provides a collection of `spawn` functions.
...
This action is supported by the [Lifecycle built-in capacity](./aop/Lifecycle.html) that the `killMe` function.
